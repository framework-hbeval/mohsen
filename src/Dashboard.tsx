    ...
    <div className="fu">
      ...
      {merchant?.plan !== "pro" && (
        <div style={{ background: "linear-gradient(135deg,rgba(0,212,168,.08),rgba(0,153,204,.05))", border: "1px solid rgba(0,212,168,.2)", borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 160 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: "var(--t1)" }}>جرّب محسِّن Pro</div>
      ...
      {total === 0 && (
        <div className="card" style={{ textAlign: "center", padding: "52px 20px" }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 800, color: "var(--t1)", marginBottom: 8 }}>لا توجد منتجات بعد</div>
          <div style={{ fontSize: 13, color: "var(--t2)", marginBottom: 22, lineHeight: 1.7 }}>اضغط مزامنة لجلب منتجاتك من سلة</div>
          <button className="btn-p full" onClick={onSync} disabled={syncing}>
            {syncing ? <><span className="sp spin" style={{ width: 14, height: 14 }} /> جاري...</> : "🔄 مزامنة الآن"}
          </button>
          {syncMsg && <div style={{ fontSize: 12, color: syncMsg.includes("✅") ? "var(--ac)" : "var(--danger)", marginTop: 12 }}>{syncMsg}</div>}
        </div>
      )}
    </div>
  );
...
// ─── Settings ─────────────────────────────────────────────────────────────────
function Settings({ merchant, products, onSync, syncing, syncMsg, onUpgrade }: any) {
  return (
    <div className="fu">
      <div style={{ marginBottom: 18 }}>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 13, color: "var(--t2)", marginBottom: 14, lineHeight: 1.6 }}>
          جلب أحدث بيانات المنتجات من سلة وإعادة حساب درجات الجودة
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <button className="btn-p" onClick={onSync} disabled={syncing}>
            {syncing ? <><span className="sp spin" style={{ width: 14, height: 14 }} /> جاري...</> : "🔄 مزامنة الآن"}
          </button>
        ...
        {syncMsg && <div style={{ marginTop: 10, fontSize: 13, color: syncMsg.includes("✅") ? "var(--ac)" : "var(--danger)" }}>{syncMsg}</div>}
      </div>

      {/* Reconnect */}
      <div className="card" style={{ marginBottom: 12 }}>
        <div style={{ fontWeight: 800, color: "var(--t1)", marginBottom: 6, fontSize: 14 }}>🔗 إعادة ربط المتجر</div>
        <div style={{ fontSize: 13, color: "var(--t2)", marginBottom: 14 }}>في حال انتهاء صلاحية التصريح</div>
    ...
    <div className="modal-bg"
      ...
      <div style={boxStyle}>
        ...
        <div style={{ padding: "24px 20px 18px", textAlign: "center", borderBottom: "1px solid var(--b1)", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ fontSize: 13, color: "var(--t2)", lineHeight: 1.6 }}>احصل على جميع أدوات التحسين المتقدمة</div>
          <div style={{ display: "inline-flex", alignItems: "baseline", gap: 4, marginTop: 16, padding: "10px 20px", background: "rgba(0,212,168,.06)", border: "1px solid rgba(0,212,168,.15)", borderRadius: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: "var(--t3)" }}>/ شهرياً</div>
              <div style={{ fontSize: 10, textDecoration: "line-through", color: "var(--t3)" }}>399 ر.س</div>
            </div>
          </div>
        </div>
...
export default function Dashboard() {
  ...
  // ── الكشف عن الموبايل بـ JavaScript — الحل الوحيد الموثوق ──────────────
  ...
  if (!merchantId && !loading) return (
    <div style={{ minHeight: "100dvh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)", padding: 20 }}>
      ...
      <div className="card fu" style={{ textAlign: "center", padding: "48px 28px", maxWidth: 380, width: "100%", border: "1px solid rgba(0,212,168,.15)" }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 900, color: "var(--ac)", marginBottom: 8 }}>محسِّن</div>
        <div style={{ fontSize: 14, color: "var(--t2)", marginBottom: 26, lineHeight: 1.7 }}>
          منصة ذكاء اصطناعي لتحسين متجرك في سلة وزيادة مبيعاتك
        </div>
  ...
  return (
    <div dir="rtl" style={{ display: "flex", minHeight: "100dvh", background: "var(--bg)", fontFamily: "var(--sans)", width: "100%" }}>
      ...
      {!isMobile && (
        <aside style={{
          width: 220, background: "var(--surf)", borderLeft: "1px solid var(--b1)",
          display: "flex", flexDirection: "column",
          position: "fixed", top: 0, right: 0,
          height: "100dvh", padding: "20px 10px",
          overflowY: "auto", zIndex: 100,
        }}>
          <div style={{ padding: "4px 8px 18px", borderBottom: "1px solid var(--b1)", marginBottom: 14 }}>
            <div style={{ fontSize: 22, fontWeight: 900, color: "var(--ac)" }}>محسِّن</div>
          ...
          <div style={{ borderTop: "1px solid var(--b1)", paddingTop: 12 }}>
            <button onClick={syncProducts} disabled={syncing}
              style={{ width: "100%", padding: "9px 12px", borderRadius: 10, border: "1px solid var(--b2)", background: "transparent", color: "var(--t3)", cursor: syncing ? "not-allowed" : "pointer", fontSize: 12, fontFamily: "var(--sans)", display: "flex", alignItems: "center", gap: 8, justifyContent: "center", fontWeight: 600 }}>
      ...
      {isMobile && (
        <header style={{
          ...
          background: "var(--surf)", borderBottom: "1px solid var(--b1)",
          padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between",
          width: "100%",
        }}>
          <div style={{ fontSize: 20, fontWeight: 900, color: "var(--ac)" }}>محسِّن</div>
          {merchant && (
      ...
      <main style={{
        flex: 1,
        width: "100%",
        // Desktop: يترك مكاناً للـ Sidebar الثابت
        // Mobile: يملأ الشاشة بالكامل بدون أي margin
        marginRight: isMobile ? 0 : 220,
        padding: isMobile ? "68px 14px 76px" : "28px 26px 40px",
        minHeight: "100dvh",
        overflowX: "hidden",
      ...
      {isMobile && (
        <nav style={{
          ...
          background: "var(--surf)", borderTop: "1px solid var(--b1)",
          display: "flex", alignItems: "center", justifyContent: "space-around",
          padding: "0 4px", width: "100%",
        }}>
          {navItems.map(item => (
            <button key={item.k} className={`bn-btn ${nav === item.k ? "on" : ""}`}
              ...
              {item.badge > 0 && (
                <span style={{ position: "absolute", top: 4, right: "calc(50% - 16px)", background: "var(--danger)", color: "#fff", fontSize: 9, fontWeight: 800, fontFamily: "var(--mono)", padding: "1px 5px", borderRadius: 10, minWidth: 16, textAlign: "center" }}>
                  {item.badge}
                </span>
              )}
